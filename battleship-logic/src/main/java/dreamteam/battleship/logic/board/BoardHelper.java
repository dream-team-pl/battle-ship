...
import static dreamteam.battleship.logic.board.Direction.HORIZONTAL;
import static dreamteam.battleship.logic.board.Direction.VERTICAL;
...
public class BoardHelper {
    ...
    List<Integer> positionsBeforeRow(int position) {
        ...
        if (!itIsOnMaxBorderSite(position, VERTICAL)&&!itIsOnMinBorderSite(position, VERTICAL)) {
            addToList((position-board.size() - 1),positionsToCheckList);
            addToList((position-board.size() +1),positionsToCheckList);
        }else if (itIsOnMaxBorderSite(position, VERTICAL)) {
            addToList((position-board.size() - 1),positionsToCheckList);
        }else if (itIsOnMinBorderSite(position, VERTICAL)) {
            addToList((position-board.size() + 1),positionsToCheckList);
        }
    ...
    List<Integer> positionsInRow(int position) {
        List<Integer> positionsToCheckList = new ArrayList<Integer>();
        if (!(itIsOnMinBorderSite( position, VERTICAL))){
            positionsToCheckList.add(position - 1);
        }
         if (!(itIsOnMaxBorderSite( position, VERTICAL))){
            positionsToCheckList.add(position + 1);
        }
    ...
    List<Integer> positionsToCheckAtPartOfShip(int position, Ship ship, int shipPartIndex, Direction direction) {
        ...
        if (direction == HORIZONTAL) {
            if (!itIsOnMinBorderSite(position, VERTICAL) && shipPartIndex == 1) {
                positionsToCheck.addAll(positionsBeforeColumn(position));
            }
            if (shipPartIndex == ship.size() && !itIsOnMaxBorderSite(position, VERTICAL)) {
                positionsToCheck.addAll(positionsAfterColumn(position));
            }
            positionsToCheck.addAll(positionsInColumn(position));
        }else if (direction == VERTICAL) {
            if (!itIsOnMinBorderSite(position, HORIZONTAL) && shipPartIndex == 1) {
                positionsToCheck.addAll(positionsBeforeRow(position));
            }
            if (shipPartIndex == ship.size() && !itIsOnMaxBorderSite(position, HORIZONTAL)) {
